...
class NeuralModel:
    """
    This class gives the general structure to run hybrid models. Hybrid model here 
    refers to the article : ##ref.
    More precisely, three different concepts are used in this class :
    - First, this class contains all information on dataset we use. It could be 
    experimental or generated by cobra. It could also be load from a training_file. The 
    MetabolicDataset class help us to get the flux data and the metabolic information
    such as stoichiometry.
    - Then, this class contains an mechanistic model stored in the model attribute. 
    - Finally, the class NeuralNetwork manage the preprocessing on dataset, train, test 
    and evaluate model with a nice diversity of parameters. Those methods will depend on
    the model type. We provide a child class for each model type, where adaptation are
    ...
    def get_parameter(self, verbose=False):
        """
        This method create a MetabolicDataset object and uses it to fill attribute
    ...
    def train_evaluate(self, verbose=False):
        """
        This method takes care of all training steps. It format the input depending on 
    ...
    def train_model(self, X_train, Y_train, X_test, Y_test, verbose=False):
        ...
        # - y_train, y_test: y values for training and tets sets
        # - o_train, l_train: objective and loss for training set
        # - o_test, l_test: objective and loss for training set
    ...
    def evaluate_model(self, x, y_true, verbose=False):
        ...
        # The model predict all the flux, but there not all given as reference values.
        # Consequently, y_pred and y_true could have different number of columns.
        ...
        loss = self.get_loss_evaluate(x, y_true, y_pred, verbose=verbose)
    ...
    ## All these functions could fill attributes during the instantiation of a metabolic model !!!
